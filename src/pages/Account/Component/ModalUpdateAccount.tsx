...
export const ModalUpdateAccount = ({ handleClose, data, refetch }: IProps) => {
  ...
  form.setFieldsValue({
    email: data?.email,
    password: showPassword(data?.password2Way),
  });

  const handleSubmit = async (payload: any) => {
    try {
      await updateAccount(data.id, payload);
      refetch();
      handleClose();
    } catch (error) {
